/******/ (() => { // webpackBootstrap
/*!*******************************************!*\
  !*** ./src/pointer-domain-search/view.js ***!
  \*******************************************/
/**
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  searchForms.forEach(form => {
    ...
    function performSearch() {
      ...
      let selectedTlds = Array.from(tldCheckboxes).filter(checkbox => checkbox.checked).map(checkbox => checkbox.value);
    ...
    function displayResults(results) {
      ...
      const resultsHtml = Object.entries(results).map(([domain, available]) => {
        const availableClass = available === '1' || available === 1 ? 'pointer-domain-search-result-available' : 'pointer-domain-search-result-unavailable';
        const availableText = available === '1' || available === 1 ? 'Διαθέσιμο' : 'Μη διαθέσιμο';

        // Προσθήκη κουμπιού για αγορά αν είναι διαθέσιμο και επιτρέπεται από τις ρυθμίσεις
        const buyButton = (available === '1' || available === 1) && showBuyButton ? `<a href="https://www.pointer.gr/domain-names/search?domain-name=${domain}" target="_blank" class="pointer-domain-search-buy-button">Αγορά</a>` : '';
        return `
